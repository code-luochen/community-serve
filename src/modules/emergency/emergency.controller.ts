...
export class EmergencyController {
    ...
    async create(@Req() req: RequestWithUser, @Body() createDto: CreateEmergencyDto) {
    ...
    async handleEmergency(
        ...
        return await this.emergencyService.handleEmergency(+id, currentUserId, updateDto);
    }

    @Get()
    @ApiOperation({ summary: '查询求助记录列表' })
    async findAll(@Req() req: RequestWithUser, @Query() query: QueryEmergencyDto) {
        const userRole = req.user.role;
        const currentUserId = req.user.userId;
